...
/*1:N일경우 1에 해당하는 부분이 join 앞에 오도록 설정
  실행순서
  1. from 게시글 제목을 조회한 뒤 작성자 정보 조회
  작성자가 1이므로 join 앞에 온다.
  3. select 작성자를 조회 해야 하므로 select u.*로 작성자 전체 정보 조회*/
select u.* from tbl_user u join tbl_post p
/* 2. on 3이 포함된 게시글 및 사용자와 작성자의 id가 같아야함*/
...
/*2. join만 사용*/
select distinct c.* from tbl_child c join tbl_apply a
on a.child_id = c.id
join tbl_field_trip f
on a.field_trip_id = f.id
where f.title like '%매미 잡기 체험학습%';


/*체험학습을 2개 이상 신청한 아이의 정보와 부모의 정보 모두 조회*/
select c.*, p.* from tbl_apply a join tbl_child c
on a.child_id = c.id
join tbl_parent p
on c.parent_id = p.id
group by c.id, p.id
having count(a.child_id) >= 2;

/*체험학습을 2개 이상 신청한 아이의 정보와 부모의 정보 모두 조회*/
select distinct c.*, p.* from tbl_parent p join tbl_child c
on p.id = c.parent_id
where c.id in
(select child_id
            from tbl_apply a
...
where count = (select MIN(count) from tbl_field_trip);

select t.title, t.content
from tbl_field_trip t
where t.count in
(select min(f.count) from tbl_field_trip f);

/*
in: 뒤에 값이 여러개일 경우 or로 연결 할 수 없기 때문에 in을 쓴다
=: =뒤에 값이 하나일 경우 앞의 행에서 차례로 검사후 출력
*/

/*평균 참가자(지원자) 수보다 적은 체험학습의 제목과 내용 조회*/
SELECT title, content
FROM tbl_field_trip
WHERE count < (SELECT AVG(count) FROM tbl_field_trip);
